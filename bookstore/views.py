...
def home(request):
    ...
    d_orders=orders.filter(status='Delievered').count()
...
#         return redirect('/') 
#     context={'form':form}
#     return render (request, 'bookstore/my_ order_form.html' ,context)
      ...
      if request.method=='POST':
        ...
        if form.is_valid():
          ...
          if result['success']:
            ...
            messages.success(request, username + 'Created sucussfully!')
            return redirect('login')

          else: 

            messages.error(request,  'unvlid Recaptcha please try again !')
